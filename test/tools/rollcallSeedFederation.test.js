...
async function stakeRosterEntry(entry, already, tip0){
...
async function stakeRosterKeys(){
    ...
    for (const entry of roster) await stakeRosterEntry(entry, already, tip0)
...
async function continueActivation(tip, lastTip, stalls, missing){
    ...
    return { lastTip, stalls }
}

function printFederation(res, tip){
...
function federationWeights(roster, res, byPubkey){
    ...
    return { total, signingWeight, at2Present, outsiderWeight }
}

async function reportReadyFederation(){
    const roster = rc.federationRoster()
    ...
    const { total, signingWeight, at2Present, outsiderWeight } = federationWeights(roster, res, byPubkey)
...
describe('ROLLCALL: seed the four-source acceptance federation', function () {
    before(setUpFederation)

    it('stakes each roster key from its own source address', stakeRosterKeys)
    it('reports a federation the acceptance suites can drive', reportReadyFederation)
})
