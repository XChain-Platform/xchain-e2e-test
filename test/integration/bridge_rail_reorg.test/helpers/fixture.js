...
async function setUp(ctx, bind) {
    if (!provider) throw new Error('bridge test entry did not load');
...
function install(bind) {
    ...
    beforeEach(async function () {
        this.timeout(0);
        if (provider.beforeEach) await provider.beforeEach.call(this);
    });
    afterEach(function () {
        if (provider.afterEach) provider.afterEach.call(this);
    });
}
