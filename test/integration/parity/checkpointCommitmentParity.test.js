 ...
 *
 * Phase 2 makes the quorum-signed checkpoint canonical (and every ANCHOR v7 bundle
 ...
 *      across the hub engine, the SDK verifier, and the indexer's v7 SECTION verifier.
...
// (no `this`), so invoke it directly off the prototype. `d` is ONE v7 bundle section,
...
function fixtures(net, snapshotBlock, withRoots) {
    ...
    const d = {
        // A v7 SECTION is root-bearing by construction: the publisher skips a null-root
...
describe('SPV Phase 2: CHECKPOINT_COMMITMENT cross-service parity', function () {
    ...
    it('post-flag-day: hub == SDK == indexer ANCHOR v7 section canonical (root suffix present)', function () {
        ...
        assert.strictEqual(hubC, idxC, 'hub vs indexer ANCHOR v7 section canonical drift');
    ...
    it('pre-flag-day: hub == SDK == indexer canonical, and the root suffix is absent', function () {
        ...
        // and the row carries null roots. No v7 section is ever cut here (a rootless row is
        // skipped, D8), so the indexer side is _canonical's shared rootless base, which the
        // archive leg still signs and which the v7 branch extends.
