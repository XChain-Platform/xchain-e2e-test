 ...
 **********************************************************************
 *
 * ANCHOR version expectations plus the v0 bundle wire parser.
 *
 * The version set RESTARTED (anchor-v0-single-wire.md D2): v0 is the checkpoint
 * bundle (yesterday's v7, byte-for-byte, only the version byte moved), v1 is
 * the archive head (yesterday's v6: checkpoint wrapper + archive segment +
 * publisher tail), v2 is the unchanged continuation chunk. A new
 * `ANCHOR_ACTIVATION` consensus height gates every ANCHOR mined below it as
 * `invalid: ANCHOR before activation`; at or above it only 0/1/2 exist, so any
 * other version is `invalid: VERSION (unknown)`. This helper describes the
 * wire AT OR ABOVE that height.
 *
 * CHECKPOINT LEG: exactly ONE version. The checkpoint bundle (ANCHOR v0, one
 * anchor per network per cycle carrying every checkpointed chain as a section)
 * REPLACED the per-chain wires; the OLD per-chain v0/v3/v4/v5 (a different,
 * now-retired numbering) were deleted, not deprecated (anchor-bundle-per-network.md
 * D2, following the pre-launch PRICE ruling), and the indexer no longer parses
 * them. So there is no version ladder left to derive here and no roots/reward
 * branch that can move it:
 *
 ...
 *     falls back WITHIN v0 to `ATTEST_SIG_COUNT 0` (§2.5) rather than to an
 ...
 *     while `accepted` stays [0].
 *
 * ARCHIVE LEG now has exactly ONE accepted version too (D4): v1, with the
 * publisher tail ALWAYS appended. The old tail-less legacy v1 wire and v6 (v1 +
 * tail) are both retired; a degraded attestation round now falls back WITHIN v1
 * to `ATTEST_SIG_COUNT 0` rather than to a second, tail-less version.
 * `rewardActive` still says whether an `anchor_archive` reward should follow;
 * it no longer selects a version. Callers assert membership in `accepted`,
 * never equality.
 *
...
function expectedCheckpointAnchor(cp, opts = {}){
    ...
    return {
        ...
        // path stays on v0 with a zero-signature tail.
        fallback:  null,
        accepted:  [0],
        describe: 'checkpoint bundle v0 (snapshot_block ' + snapshotBlock + '/' + network +
                  ', roots ' + (rootBearing ? 'on' : 'off') +
                  ', reward-derivation ' + (rewardActive ? 'on' : 'off') + ')'
    };
}

// The ARCHIVE-anchor expectation for the same checkpoint row: always v1 (D4).
// The tail is now ALWAYS appended, so the old tail-less wire and v6 are both
// retired. `rewardActive` still reports whether an `anchor_archive` reward
...
function expectedArchiveAnchor(cp, opts = {}){
    ...
    return {
        rewardActive,
        preferred: 1,
        // No cross-version fallback exists any more; a degraded attestation
        // round stays on v1 with ATTEST_SIG_COUNT 0.
        fallback:  null,
        accepted:  [1],
        describe: 'archive anchor v1 (snapshot_block ' + snapshotBlock + '/' + network +
                  ', reward-derivation ' + (rewardActive ? 'on' : 'off') + ')'
    };
}
...
// Fixed fields a v0 section carries before its (PUBKEY, SIG) pair list:
...
const V0_SECTION_FIXED_FIELDS = 13;

// Parse an ANCHOR v0 bundle payload into its header, sections and publisher
...
// chain. Returns null for anything that is not an ANCHOR v0.
//
...
function parseAnchorV0(payload){
    ...
    return {
        version: 0, network: f[2], snapshot_block: Number(f[3]),
...
function findBundleSectionRows(rows, ledgerHash){
    ...
    if(!seed) return [];
    return (rows || [])
        .filter(r => Number(r.version) === 0 && String(r.action_index) === String(seed.action_index))
