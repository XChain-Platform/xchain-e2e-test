...
describe('AB3: safety preserved, the armed node holds inside the horizon window under a pinned height', function () {
    ...
    const ctx = { venue: null, btc: null, coin: 'BTC', spaced: null, block: null, pin: null, held: null, hold: null }
...
async function placeWindowAndPin (ctx) {
    ...
    await drive.levelIndexers(ctx.venue)
    ...
    assert.strictEqual(ctx.block.height, tip + 1)
    ctx.window = window
}
