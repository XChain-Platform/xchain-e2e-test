...
async function setupFallthrough() {
...
async function teardownFallthrough() {
...
async function createStarvedRequest() {
...
async function findRequest(sinceAction) {
...
async function injectInertRow(requestId, requestRow, honestEffective) {
    // ---- the injection ------------------------------------------------
    //
...
async function waitForPairedRows(requestId) {
    return untilOrClearDogeStall(async () => {
        const counts = []
        for (const ix of venue.indexers) {
...
async function waitForBoundRows(requestId) {
    // ---- the applier runs -----------------------------------------------
    return untilOrClearDogeStall(async () => {
...
function readSkipEvidence(requestId) {
    ...
    return venue.indexers.map((ix) => {
        ...
        return { ix, tail, lines, matching, wantSkip }
    })
}

async function readInertEvidence(requestId, honestEffective) {
    ...
    for (const ix of venue.indexers) {
        ...
        evidence.push({ ix, rows, still })
    }
    return evidence
}

async function readApplyCounts(bindBlock) {
    ...
    for (const ix of venue.indexers) {
        const rows = await queryDb(venue, ix.indexerDbName,
            ...
            [bindBlock, 'ATTEST'])
        counts.push({ ix, n: Number(rows && rows[0] ? rows[0].n : -1) })
    }
    return counts
}
...
async function verifiesFallthrough() {
    ...
    console.log('ZC4: hub ' + honest.hub + ' finalized ' + requestId.slice(0, 12) + ' with effective_time ' + honestEffective)
    await injectInertRow(requestId, requestRow, honestEffective)
    const paired = await waitForPairedRows(requestId)
    assert.ok(paired.ok, 'both mirror rows did not reach both indexers after the release: counts ' + jsonSafe(paired.counts) + '. The release drops the socket so the mirror re-bootstraps, ' + 'and attestation_responses re-pages from id 0, so a count under two means the injected ' + 'row never left the hub table.\n' + venue.logTail('indexer0'))
    console.log('ZC4: both indexers hold ' + jsonSafe(paired.counts) + ' rows for the request')
    const bound = await waitForBoundRows(requestId)
    assert.ok(bound.ok, 'the response was never applied on both indexers. The honest row is valid and ' + 'applicable, so a request left pending here is the pre-§5 behaviour: the selector kept ' + 'choosing the inert head and the applier wrote nothing.\n' + venue.logTail('indexer0') + '\n' + venue.logTail('indexer1'))
    const diffs = diffRows(bound.rows[0], bound.rows[1], APPLIED_FIELDS)
    assert.deepStrictEqual(diffs, [], 'the two indexers bound different rows or bound at different blocks: ' + diffs.join('; ') + '. The candidate order is built from two SIGNED fields, so every node walks the same ' + 'sequence and stops at the same place; a difference here is a fork.')
    ...
    assert.strictEqual(String(bound.rows[0].response_hash).toLowerCase(), String(honest.row.response_hash).toLowerCase(), 'the applier bound a response hash that is not the honest row\'s')
    for (const { ix, tail, lines, matching, wantSkip } of readSkipEvidence(requestId)) {
        assert.ok(lines.length > 0, 'indexer ' + ix.index + ' logged no SKIPPED line for ' + wantSkip + ' at all. The ' + 'injected row is the head of the candidate list, so the applier must have tried it ' + 'and refused it before reaching the honest one; no line means it was never tried, ' + 'which is the single-candidate selector.\n' + tail)
        assert.ok(matching.length > 0, 'indexer ' + ix.index + ' skipped the injected row for a reason other than "' + SKIP_REASON_PREFIX + '": ' + jsonSafe(lines) + '. Every cheaper skip in the applier ' + '(provider, status, signature format, body cap, response_hash echo) is passed by ' + 'construction, because the injected row is a clone of the honest one with only its ' + 'signers and stamp changed. A different reason means the clone drifted.')
        console.log('ZC4: indexer ' + ix.index + ' logged ' + matching.length + ' skip(s): ' + matching[0].trim())
    }
    for (const { ix, rows, still } of await readInertEvidence(requestId, honestEffective)) assert.strictEqual(still.length, 1, 'indexer ' + ix.index + ' no longer holds the injected row at effective_time ' + (honestEffective - 1) + ' (rows: ' + jsonSafe(rows.map((r) => Number(r.effective_time))) + '). A skipped row must stay ' + 'in the table: the design has no retraction path precisely because an unbound row ' + 'is harmless.')
    ...
    assert.strictEqual(JSON.parse(state.callback_payload), FIXED_BODY, 'the callback carried a body other than the one the provider served')
    console.log('ZC4 GREEN: the inert head was skipped on both indexers, the honest row bound at ' + bindBlock + ' with one ATTEST format-1 action, and the inert row is still in the mirror')
}

describe('ZC4: the applier falls through an inert row to the honest one', function () {
    this.timeout(75 * 60 * 1000)
    before(setupFallthrough)
    after(teardownFallthrough)
    it('skips the inert candidate, binds the honest row in the same block, and keeps the inert one', verifiesFallthrough)
})
