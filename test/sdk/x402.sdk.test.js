...
async function verifyConfirmedSend() {
    this.timeout(300000);
...
async function verifyProvisionalSend() {
    this.timeout(300000);
...
async function verifyDepositMeter() {
    this.timeout(300000);
...
async function verifyGatedFile() {
    this.timeout(300000);
...
async function verifyDispenserAccess() {
    this.timeout(300000);
...
async function verifyPolicyLimit() {
    this.timeout(120000);
    const store = new X402Storefront({
...
describe(`x402 agent payments: SDK driven (${COIN})`, function () {
    before(prepareX402);
    after(cleanX402);
    it('1. confirmed SEND loop: 402 -> pay -> 1-conf -> 200 with receipt', verifyConfirmedSend);
    it('2. 0-conf loop: provisional grant from the mempool, sweeper promotes after a block', verifyProvisionalSend);
    it('3. deposit-metered: fund once, two calls debit, third exhausts back to 402', verifyDepositMeter);
    it('4. gated-FILE storefront: pay, receive the key payload, decrypt, byte-match', verifyGatedFile);
    it('6. dispenser hold-to-access: challenge advertises the dispenser; holding the tick grants access', verifyDispenserAccess);
    it('5. AgentSession policy refuses an over-cap price before any payment leaves', verifyPolicyLimit);
});
