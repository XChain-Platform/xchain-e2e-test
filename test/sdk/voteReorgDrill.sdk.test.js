...
async function prepareReorgVote() {
...
async function verifyBallotHistory() {
...
async function verifyReconfirmation() {
...
describe('[sdk] VOTE re-ballot reorg drill (orphaned re-vote restores the prior ballot)', function () {
    this.timeout(0);
    before(prepareReorgVote);
    it('a re-vote appends a second ballot set; the latest set is the current ballot', verifyBallotHistory);
    it('orphaning the re-vote block restores the prior ballot as current', verifyRollback);
    it('the orphaned re-vote re-confirms from the mempool and becomes current again', verifyReconfirmation);
});
