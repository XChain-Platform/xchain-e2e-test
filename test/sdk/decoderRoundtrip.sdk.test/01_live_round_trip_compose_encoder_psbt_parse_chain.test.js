...
let sdk, issuer, recipient, tick, liveSetup;

function initializeSdk () {
    if (!sdk) sdk = makeSdk({ compactAddresses: false });
}

async function loadLiveSetup () {
    if (liveSetup) return true;
    if (!haveConnectors()) return false;
...
function registerLiveBlock (registerTests) {
    describe('[sdk] decoder.parse vs the live chain', function () {
        ...
        describe('live round-trip: compose -> encoder PSBT -> parse -> chain', function () {
            before(async function () {
                if (!await loadLiveSetup()) this.skip();
            });
            registerTests();
        });
    });
