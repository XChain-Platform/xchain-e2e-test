...
let sdk, issuer, voterA, voterB, voterC, outsider, tick, voteSetup;

async function prepareVote() {
    if (voteSetup) return voteSetup;
    ...
    voteSetup = { sdk, issuer, voterA, voterB, voterC, outsider, tick };
    return voteSetup;
}
...
async function createApprovalPoll() {
    ...
    return submit(sdk,
        { action: 'VOTE', params: {
            version: 0, tick, endBlock, options: 'YES,NO', maxSelections: 1,
            tallyMode: 'approval', weightMode: 'balance', question: 'Adopt proposal A?'
        } },
        { pubkey: issuer.address, change: issuer.address },
        submitOpts({ wif: issuer.wif })
    );
...
async function submitApprovalBallot(voter, pollIndex, choice) {
    return submit(sdk,
...
async function submitRejectedBallot(voter, pollIndex) {
    return submit(sdk,
        { action: 'VOTE', params: { version: 1, pollRef: pollIndex, ballot: '0' } },
        { pubkey: voter.address, change: voter.address },
        submitOpts({ wif: voter.wif, requireValid: false }));
}

function registerApprovalPoll() {
    let pollIndex;
    it('VOTE v0 create writes a polls row', async function () {
        const res = await createApprovalPoll();
    ...
    it('three single-option ballots each write a votes row', async function () {
        ...
        for (const [voter, choice] of [[voterA, 0], [voterB, 1], [voterC, 0]]) {
            const res = await submitApprovalBallot(voter, pollIndex, choice);
    ...
    it('hold-to-vote gate: a non-holder ballot writes no votes row', async function () {
        ...
        const res = await submitRejectedBallot(outsider, pollIndex);
...
describe('[sdk] VOTE token-weighted governance', function () {
    this.timeout(0);
    before(prepareVote);
    describe('approval / balance poll', registerApprovalPoll);
    describe('split multi-select poll', registerSplitPoll);
});
