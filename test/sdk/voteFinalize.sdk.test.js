...
let sdk, issuer, voterA, voterB, finalizeSetup;

async function prepareFinalizeVote() {
    if (finalizeSetup) return finalizeSetup;
    ...
    finalizeSetup = { sdk, issuer, voterA, voterB };
    return finalizeSetup;
}

async function verifyTimeFinalization() {
...
async function verifyEarlyDecision() {
...
async function verifyFailedQuorum() {
...
describe('[sdk] VOTE v2 finalization', function () {
    this.timeout(0);
    before(prepareFinalizeVote);
    it('time-finalize: a closed poll freezes a winner + poll_results rows', verifyTimeFinalization);
    it('early-decide: a majority ballot closes the poll before END_BLOCK', verifyEarlyDecision);
    it('failed_quorum: a poll missing MIN_VOTERS closes with no winner', verifyFailedQuorum);
});
