...
async function reorgContext(addrABlock) {
    ...
    return { tipBefore, orphanHash, miner }
}

async function overtakeOriginalTip(tipBefore, addrABlock, miner) {
    const need = tipBefore - (addrABlock - 1) + 2
    for (let i = 0; i < need; i++) await nodeConnector.generateBlock(miner, [])
}

async function waitForAddressCreditRemoval(addrA) {
...
async function reclaimOrphanedAddressId() {
    ...
    try {
        const { tipBefore, orphanHash, miner } = await reorgContext(addrABlock)
        ...
        await overtakeOriginalTip(tipBefore, addrABlock, miner)
        ...
        const addrAGone = await waitForAddressCreditRemoval(addrA)
        ...
        // would otherwise re-create it with a NULL/stale block_index, poisoning ^id).
...
describe(ADDRESS_REORG_TITLE, function () {
    this.timeout(0)
    before(function () { if (global.COIN_CODE === 'DOGE') this.skip() })
    it('an orphaned new-address id is reclaimed by rollback, so a later DIFFERENT address reuses it (no ^id divergence)', reclaimOrphanedAddressId)
})
