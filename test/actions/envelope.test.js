...
describe('TAPROOT ENVELOPE', () => {
    ...
    before(skipUnlessEnvelopeSupported)

    describe('a compressed FILE in one commit/reveal pair', () => {
        before(publishCompressedFile)
    ...
    describe('a compressed FILE in one commit/reveal pair', confirmCompressedFile)
    describe('a compressed FILE in one commit/reveal pair', verifyCompressedPayload)
    describe('a compressed FILE in one commit/reveal pair', verifyRevealFunding)
    describe('a compressed FILE in one commit/reveal pair', verifyServedPayload)
})
...
let sent = null
let onChain = null
let publicationSetup = null

function skipUnlessEnvelopeSupported(){
    if (!envelopeHelper.envelopeSupported()) this.skip()
}

async function publishCompressedFile(){
    if (!publicationSetup) publicationSetup = (async () => {
    ...
    await publicationSetup
}

function confirmCompressedFile(){
    before(publishCompressedFile)
...
function verifyCompressedPayload(){
    before(publishCompressedFile)
