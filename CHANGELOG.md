...
### Added
- `test/regression/xcallDeliveryLag.regression.test.js` — regression guard for the cross-chain-call live/replay fork under hub delivery lag. The full-stack XCALL suites run single-process with zero delivery lag, so a live node and a replaying node coincide and the suites are structurally false-green for this class: a relayed `cross_chain_calls` row gates injection on `effective_time <= block_time`, and if the row arrives even one block after the eligible block the live node injects late, shifting its action-index counter (`EMITTER_ACTION_INDEX` is in the `call_id` preimage) and desynchronizing every subsequent call. This test loads the real hub `CrossChainCallEngine` (so it tracks the actual relay-margin policy, not a copy) and models the deterministic injection rule with a synthetic one-block delivery lag, asserting that: with no margin the live and replay paths inject at different blocks and derive different `call_id`s (documenting the fork the fix prevents); with the real forward `effective_time` margin both paths inject at the same block with the same `call_id`; and the margin exceeds a realistic one-block delivery lag on every chain. Skips cleanly if the hub source is not reachable (honors `XCHAIN_HUB_PATH`).
- `test/integration/multiHubGovernanceCapReload.integration.test.js` — federation regression guard for governance-driven capability config hot-reload. Boots a 3-validator hub set, drives a `CAPABILITY_PRICE_MIN_STAKE` proposal through the full propose → vote → tally flow, and asserts that **every** hub — not just the deterministic tally leader — converges on the new `getMinStake('price')` after the proposal passes. Tallying is single-leader, so only the leader runs `_tallyProposal` and applies the change directly; followers learn the outcome solely via the `GOV_RESULT` broadcast. If that path fails to trigger a hot-reload, followers keep serving the stale threshold while the leader serves the new one — and since each hub feeds its own `getMinStake()` to the indexer when locking the quorum validator set, the federation would compute different qualified sets for the same block and break PBFT agreement. Skips when `HUB_DB_USER`/`HUB_DB_PASS` are unset (same gate as the other multi-hub integration tests).
...
- `test/transactionHelper.js` — the two-transaction reveal flow now also drives `P2WSH` (previously only `P2SH`); the encoder builds the witness spending PSBT and the existing finalizer detects P2SH vs P2WSH per input, so large payloads can fan out across multiple witness-revealing inputs.
...
- `test/sdk/misc.sdk.test.js` — new assertion that `sdk.getStatus()` exposes the decoder-tip reference and indexer→decoder lag the explorer now returns. The test requires `decoder_tip` and `decoder_lag_blocks` maps to be present with an entry for every coin in `last_block`, and for each coin asserts that when `decoder_tip` is non-null the tip is a number at or ahead of `last_block` and `decoder_lag_blocks` is a non-negative number (and that `decoder_lag_blocks` is `null` whenever `decoder_tip` is). It also requires at least one coin to report a non-null tip, since the e2e regtest stack co-locates the decoder, so a silent degrade to all-null would be a regression. The test deliberately asserts nothing about the coin node's chain tip — these fields cover the indexer→decoder slice only (the explorer never talks to a coin node), and the chain→decoder gap is surfaced by the decoder's own `health()` RPC, not `/status`. The existing suite only checked the indexer's own `last_block` position and would not have caught a missing lag signal.

### Fixed
- `src/XChainHubConnector.js`, `test/initialCheck.test.js` — when every hub endpoint is unreachable, `_call()` now records a per-endpoint failure summary on `connector.lastFailures` (a `url → code` entry per failed endpoint) instead of discarding it after a one-line `console.warn` per URL, and the hub-discovery bootstrap fallback embeds that summary in its "Can't connect to the XChain Hub" error. Previously the only surviving signal was the generic message with no indication of which endpoints were tried or why each failed, so a multi-endpoint outage left the operator nothing to act on. The per-URL warnings are retained — this only enriches the aggregated error, and the connector's `null`/degraded return contract is unchanged.
...
### Changed
- `README.md`, `test/initialCheck.test.js`, `test/integration/setup/bootstrap-env.test.js`, `test/helpers/multiValidatorHubHelper.js` — renamed the indexer service-host environment variable `INDEXER_HOST` → `INDEXER_URL` to match the `<SERVICE>_URL` convention every other service uses (`NODE_URL`, `ENCODER_URL`, `DECODER_URL`, `REGTEST_MINER_URL`). Previously the suite read `process.env.INDEXER_HOST` while documenting every sibling as `*_URL`, so an operator following the pattern set `INDEXER_URL` (ignored), the indexer was treated as unconfigured, hub fallback engaged, and a double failure aborted the run with no pointer to the wrong variable name. Pure rename — no logic, default, or behavior change.
...
- `package.json` — raised the `bitcoinjs-lib` dependency floor from `^6.1.5` to `^6.1.7`, matching the version already declared by the encoder, decoder, UTXO-tracker, and SDK services. All resolved to `6.1.7` at runtime, but the suite's lower floor meant an isolated `package-lock.json` regeneration could pick up an older `6.1.x` patch than the services under test — undermining the round-trip fidelity the e2e suite exists to guard, for a library that owns PSBT and script serialization. The lockfile is regenerated; no resolved versions or source code change.
