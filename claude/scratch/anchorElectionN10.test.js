...
describe('ANCHOR election live: multi-validator per-chain publishers (DOGE regtest)', function () {
    ...
    before(async function () {
        ...
        for (const hub of mvh.hubs) {
            const snap = {
                ...
                async getWeightSnapshot(){ return { validators: pubkeys.map(function(pk,i){ return { pubkey: pk, source: "s"+i, weight: "1000" }; }) }; },
            };
    ...
    it('archive: the per-block leader collects 2f+1 live co-signatures and XANC_FINALIZED back-fills every hub', async function () {
        ...
        const sigs = JSON.stringify(identities.slice(0, 7).map(id =>
        ...
        await sleep(15000); // SIGN round + publish + FINALIZED (N=10 co-sign window)
        ...
        assert.ok(sigCount >= 7, 'v1 carries 2f+1 live co-signatures (got ' + sigCount + ')');
